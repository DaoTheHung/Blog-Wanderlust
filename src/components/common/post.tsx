...
export const Post = ({data}:TData) => {
	...
	const convertToSlug = (input:string) => {
		...
		const replacedStr = lowercaseStr.replace(/[-.,\/#!$%\^&\*;:{}=\-_`~()]/g,'').replace(/\s/g, '-');
	...
	return (
		<div className="w-full flex justify-center mx-auto">
			<div className="flex w-[1251px] justify-start px-4 lg:px-auto gap-10  flex-wrap">
				{data.map((item) => (
					<div key={item.id} className="md:w-[200px] lg:w-[375px]  ">
						<div className="flex flex-col gap-7 items-center">
							...
							<div className="flex  flex-col items-center gap-3">
								...
								<Link href={`${pathname}/${convertToSlug(item.title)}`} className="font-bold text-[#444] uppercase cursor-pointer hover:text-[#c39f76]">
									{item.title}
								</Link>
